...
class BeamSearchScorer(BeamScorer):
    ...
    def process(
        ...
        next_scores: List[torch.LongTensor],
        ...
        for _ in range(num_models):
            next_beam_scores.append(torch.zeros((batch_size, self.group_size), dtype=next_scores[0].dtype, device=device))
    ...
    # raw function from transformers
    def finalize(
        self,
        input_ids: torch.LongTensor,
...
class FusionModel(nn.Module):
    def __init__(self, model, weights, f_type):
        super().__init__() 
        self.model = model
    ...
    def __call__(self, past_key_values, **model_inputs):
        ...
        with torch.no_grad():
            ...
            with self.model.disable_adapter():
                outputs[-1] = self.model(past_key_values=past_key_values[2] if past_key_values is not None else None, **model_inputs)
        output = outputs[0]
    ...
    def generate(
        ...
        **kwargs,
        ...
        generation_mode = self.model._get_generation_mode(generation_config, assistant_model)
    ...
    def f_value(self, logp_list):
        ...
        if self.f_type == 'reverse_kl' or self.f_type == "jsd":
            return torch.sum(torch.stack([self.weights[idx]*logp_list[idx] for idx in range(self.num_models-1)]), dim=0)
        elif self.f_type == 'forward_kl':
            calc_lst = []
            for idx in range(self.num_models-1):
                if self.weights[idx] != 0:
                    calc_lst.append(-logp_list[idx]+np.log(self.weights[idx]))
            return -torch.logsumexp(torch.stack(calc_lst), dim=0)
        elif "-divergence" in self.f_type:
            parts = self.f_type.split('-')
            alpha = float(parts[0]) if parts else None
            calc_lst = []
            for idx in range(self.num_models-1):
                if self.weights[idx] != 0:
                    calc_lst.append(-logp_list[idx]*alpha+np.log(self.weights[idx]))
            return -torch.logsumexp(torch.stack(calc_lst), dim=0)
        else: raise NotImplementedError
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        for _ in range(self.num_models):
            beam_scores.append(torch.zeros((batch_size, num_beams), dtype=torch.float, device=input_ids.device))
        ...
        while True:
            ...
            if model_kwargs["past_key_values"] is not None:
                for model_idx in range(self.num_models):
                    model_kwargs["past_key_values"][model_idx] = self.model._reorder_cache(model_kwargs["past_key_values"][model_idx], beam_idx)
