...
class ScriptArguments:
    ...
    misspecification: Optional[bool] = field(
        default=False,
        metadata={
            "help": "whether to use misspecification, i.e., only train the lm_head and freeze the rest of the model"
        },
    )
...
def prepare_data(
    ...
    for sample in ds:
        if choose_type == "random":
            ...
            idx0 = np.argmax(r)
            idx1 = np.argmin(r)
        elif choose_type == "mixed_max_min":
            len_sample = len(sample["rewards"])
            assert len_sample % 2 == 0
            if random.random() <= 0.3:
                idx0 = np.argmax(sample["rewards"][:len_sample//2])
                idx1 = np.argmin(sample["rewards"][:len_sample//2])
            else:
                idx0 = np.argmax(sample["rewards"][len_sample//2:])
                idx1 = np.argmin(sample["rewards"][len_sample//2:])
        else:
...
if __name__ == "__main__":
    ...
    seed_number = 42 # modify the random_seed number here!
    ...
    if script_args.misspecification:
        for param in model.transformer.wte.parameters():
            param.requires_grad = False
        for layer in model.transformer.h[:15]:
            for param in layer.parameters():
                param.requires_grad = False
    print("#"*10)
    print("model size: ", sum(p.numel() for p in model.parameters() if p.requires_grad))
    print("model size (all): ", sum(p.numel() for p in model.parameters()))
    print("#"*10)
    ...
    if seed_number != 42:
        training_args.seed = seed_number
    print(training_args)
